...
function start() {
    ...
    // Lê o grupo da URL (ex: meubolao.com/?grupo=familia)
    const urlParams = new URLSearchParams(window.location.search);
    const urlGroup = urlParams.get('grupo');
    
    setActiveGroup(urlGroup || DEFAULT_GROUP_NAME);
    renderGames();
    renderAdminResults();
    bindEvents();
...
function setActiveGroup(rawName) {
    ...
    // Trava o grupo na URL. Isso dificulta a mudança e facilita compartilhar!
    const urlParams = new URLSearchParams(window.location.search);
...
async function handleGroupName() {
    ...
    if (newGroupId !== state.groupId) {
        setActiveGroup(name);

        // Se o usuário digitar um grupo diferente, recarregamos os dados isolados
        listenToRanking();
        listenToParticipants();
        listenToAllPredictions();
...
function bindEvents() {
    ...
    els.groupName.addEventListener("change", () => handleGroupName().catch(handleFirebaseError));
    els.groupName.addEventListener("blur", () => handleGroupName().catch(handleFirebaseError));
